...
21. [Social Layer](#21-social-layer)
22. [Payment Provider Abstraction](#22-payment-provider-abstraction)
23. [Frontend Health Agent](#23-frontend-health-agent)
24. [Subscription Tiers](#24-subscription-tiers)

**Deep-Dive Documents:** [ALGORITHMS.md](ALGORITHMS.md) | [DATA_FLOW.md](DATA_FLOW.md) | [API_CATALOG.md](API_CATALOG.md) | [INTEGRATION_MAP.md](INTEGRATION_MAP.md)
...
X_CONSUMER_KEY=<from developer.x.com>            # backend/Drupal integrations
...
The frontend dev server connects to the remote Drupal API at `DRUPAL_API_URL` from `.env.local`.
...
const res = await fetch(
  `${DRUPAL_API_URL}/jsonapi/node/creator_x_profile?filter[field_x_username]=${username}&include=field_profile_picture`,
...
### Create (POST)
```typescript
const res = await fetch(`${DRUPAL_API_URL}/jsonapi/node/creator_x_profile`, {
  method: "POST",
  headers: drupalAuthHeaders(),
  body: JSON.stringify({
    data: {
...
### Update (PATCH)
```typescript
const res = await fetch(`${DRUPAL_API_URL}/jsonapi/node/creator_x_profile/${uuid}`, {
  method: "PATCH",
  headers: drupalAuthHeaders(),
  body: JSON.stringify({
    data: {
...
| `XAI_API_KEY` | Grok AI |
| `X_CONSUMER_KEY` / `X_CONSUMER_SECRET` | X API (backend/Drupal integrations) |
...
The following companion documents provide detailed coverage of specific systems. This master doc gives the overview; the deep-dives give the implementation details.

| Document | Purpose | When to Use |
|---|---|---|
| [ALGORITHMS.md](ALGORITHMS.md) | Every formula, heuristic, and computed value (21 entries) | Debugging business logic, understanding scoring/fees |
| [DATA_FLOW.md](DATA_FLOW.md) | End-to-end traces for 12 major operations | Tracing bugs across system boundaries |
| [API_CATALOG.md](API_CATALOG.md) | All 38 API routes with request/response shapes | Consuming or extending the API |
| [INTEGRATION_MAP.md](INTEGRATION_MAP.md) | External service reference (10 services) | Credential rotation, service outages, adding integrations |

---

# 21. Social Layer

The follow system enables creators to build community within the platform.

## Architecture
- **Drupal Flag module** stores follow relationships as `flagging--follow_creator` entities
- Each flagging records: `field_follower_store_id` (who followed) + `flagged_entity` (who was followed) + `field_follow_source` (rareimagery or x_import)
- **Denormalized counts** on `commerce_store`: `field_follower_count`, `field_following_count`

## Features
| Feature | Route | Description |
|---|---|---|
| Follow/Unfollow | `POST /api/social/follow` | Creates/deletes flagging, updates counts |
| Followers list | `GET /api/social/followers` | Lists followers with mutual detection |
| Picks | `GET/POST /api/social/picks` | Curated featured stores |
| Shoutouts | `GET/POST /api/social/shoutouts` | Creator-to-creator endorsements |
| X Seed Import | `GET /api/social/seed-from-x` | Cross-reference X following with platform stores |

## Mutual Detection
For each follower, a reverse query checks if the target also follows them back. See [ALGORITHMS.md](ALGORITHMS.md) §9 for details.

**Source:** `src/lib/social.ts`, `src/app/api/social/`

---

# 22. Payment Provider Abstraction

The platform supports multiple payment providers through a `PaymentProvider` interface.

## Interface
```typescript
interface PaymentProvider {
  createCheckout(params) → Promise<PaymentIntent>
  createSubscription(params) → Promise<SubscriptionIntent>
  verifyPayment(paymentId) → Promise<PaymentIntent>
...
|---|---|---|
| **Stripe** | Active (current) | `STRIPE_SECRET_KEY` |
| **X Money** | Stub (awaiting API launch) | `XMONEY_API_KEY` |

## Resolution: X Money preferred when available, falls back to Stripe.

## Fee Structure
| Fee | Amount | Trigger |
|---|---|---|
| Store setup | $5 one-time + $6 first month ($11 total) | Store creation |
| Monthly maintenance | $6/month | Recurring Stripe subscription |
| Listing fee | $0.05 per product | After 50 free products |
| Platform processing | 2.9% + $0.30 per order | Every product purchase |

See [ALGORITHMS.md](ALGORITHMS.md) §5–7 for detailed formulas.

**Source:** `src/lib/payments.ts`, `src/lib/stripe.ts`

---

# 23. Frontend Health Agent

Automated system monitoring running as a Vercel cron job every 30 minutes.

## Checks Performed
1. **Drupal reachability** — `GET /jsonapi` with 10s timeout
2. **Store page accessibility** — `GET /stores/{username}` for each creator (15s timeout, batches of 5)
3. **Product counts** — Verifies product queries work for each store
4. **Critical API routes** — `GET /api/social/picks` with 10s timeout

## Auto-Remediation
- If an approved store page returns non-200: calls `revalidatePath()` to clear Next.js cache

## Alerting
- **Critical** (Drupal down, profiles unfetchable): sends admin email with full JSON report
- **Degraded** (individual store/route issues): logged in report
- **Healthy**: no action

See [DATA_FLOW.md](DATA_FLOW.md) §10 for the complete flow.
...
Creators can define fan subscription tiers for gated content.

## Storage
- Tiers stored as JSON in `field_subscription_tiers` on `commerce_store`
- Each tier: `{ id, name, price, currency, interval, description, perks[], subscriberCount, featured }`

## Access Control
- Products can be marked `field_subscriber_only: true` with `field_min_tier` specifying minimum tier
- Theme components check subscription status before rendering gated products

## X Integration
- `field_x_subscription_tier` on creator profiles links to X Creator subscription tiers
- Managed entirely by X — platform only links to X subscription page

## Routes
| Route | Purpose |
|---|---|
| `GET /api/subscriptions/tiers` | List tiers for a store |
| `POST /api/subscriptions/tiers` | Save tier definitions |
| `POST /api/subscriptions/checkout` | Create subscription payment session |
| `GET /api/subscriptions/status` | Check user's subscription status |

**Source:** `src/app/api/subscriptions/`, `src/lib/payments.ts` (SubscriptionTier type)
